...
import { signoutUser, makeNotification, setOpenSearch } from "../redux";
...
const StyledNavbar = styled.nav`
  ...
  box-shadow: 0px 3px 3px -2px rgb(0 0 0 / 15%),
    0px 3px 4px 0px rgb(0 0 0 / 10%), 0px 1px 8px 0px rgb(0 0 0 / 8%);
...
const Navbar = () => {
  ...
  const { userDetails: user, openSearch } = useSelector((state) => state.user);
  ...
  const pushToWishlist = (uid) => {
    if (uid) {
      ...
      dispatch(
        makeNotification({
          message: "Please sign in before viewing wishlist",
          variant: "info",
          duration: 2500,
        })
      );
  ...
  const pushToCart = (uid) => {
    if (uid) {
      ...
      dispatch(
        makeNotification({
          message: "Please sign in before viewing cart",
          variant: "info",
          duration: 2500,
        })
      );
    ...
    >
      ...
      <div className={"botton__navbar"}>
        ...
        <div className={classes.NavOperationsContainer}>
          <Tooltip
            title={
              user?.uid ? `You signed in as ${user?.email}` : "login or sign up"
            }
          ...
          <Tooltip title='wishlist' aria-label='wishlist'>
            <IconButton onClick={() => pushToWishlist(user?.uid)}>
              <Badge
                badgeContent={
                  user?.wishlist?.length > 0 ? user?.wishlist?.length : 0
                }
                color='error'
              >
                <Icon name='heart' />
